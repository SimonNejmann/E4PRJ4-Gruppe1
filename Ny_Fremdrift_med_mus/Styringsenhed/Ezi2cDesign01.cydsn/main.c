...
int main()
{
    ...
    ***************************************************************************/
    ...
    for(;;)
    {
        /* SW1: Write data to EZI2C */
        if(actionSelect == ACTION_SELECT_WRITE)
        {
            /* Clear action */
            actionSelect = ACTION_SELECT_NONE;

            /* Increment members of array to be written to the EZI2C */
            for(i=1u; i<5u; i++)
            {
                i2cBufferWrite[i] += i;
            }
            ...
            I2C_UDB_M_MasterWriteBuf(EZI2C_SLAVE_ADDR, (uint8 *) i2cBufferWrite,
                                     (SLAVE_ADDR_SIZE + BUFFER_SIZE), I2C_UDB_M_MODE_COMPLETE_XFER);

            while((I2C_UDB_M_MasterStatus() & I2C_UDB_M_MSTAT_WR_CMPLT) == 0u)
            {
                /* Wait write to be completed */
            }

            /* Display data written to the EZI2C */
            LCD_Position(0u,0u);
            LCD_PrintString("WR: ");
        ...
        if(actionSelect == ACTION_SELECT_READ)
        {
            ...
            I2C_UDB_M_MasterReadBuf(EZI2C_SLAVE_ADDR, (uint8 *) i2cBufferRead,
                                    BUFFER_SIZE, I2C_UDB_M_MODE_COMPLETE_XFER);
